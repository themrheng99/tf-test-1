...
public class EntryResource {
    ...
    public ResponseEntity<EntryDTO> createEntry(@Valid @RequestBody EntryDTO entryDTO) throws URISyntaxException {
        ...
        // check if entry having blog refer, check emoji and content validity
        if (entryDTO.getBlogId() != null) {
            // extract each words in content for checking
            List<String> checkWords = Arrays.asList(entryDTO.getContent().toLowerCase().split(" "));
            if (entryDTO.isBlogPositive()) {
                ...
                if (checkWords.contains("sad") || checkWords.contains("fear") || checkWords.contains("lonely")) {
                    throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
                }
            } else {
                if (entryDTO.getEmoji().equals(Emoji.LIKE) || entryDTO.getEmoji().equals(Emoji.HAHA)) {
                    throw new BadRequestAlertException("Invalid Emoji", ENTITY_NAME, "invalidEmoji");
                }
                if (checkWords.contains("happy") || checkWords.contains("love") || checkWords.contains("trust")) {
                    throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
                }
            }
        }
    ...
    public ResponseEntity<EntryDTO> updateEntry(@Valid @RequestBody EntryDTO entryDTO) throws URISyntaxException {
        ...
        if (entryDTO.getBlogId() != null) {
            ...
            if (entryDTO.isBlogPositive()) {
                ...
                if (checkWords.contains("unhappy") ||checkWords.contains("sad") || checkWords.contains("fear") || checkWords.contains("lonely")) {
                    throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
                }
            } else {
                if (entryDTO.getEmoji().equals(Emoji.LIKE) || entryDTO.getEmoji().equals(Emoji.HAHA)) {
                ...
                if (checkWords.contains("fearless") ||checkWords.contains("happy") || checkWords.contains("love") || checkWords.contains("trust")) {
                    throw new BadRequestAlertException("Invalid Content", ENTITY_NAME, "invalidContent");
                }
            }
        }
